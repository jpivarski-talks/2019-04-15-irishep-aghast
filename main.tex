...
\item[serializable:]<2-> We use Flatbuffers as a wire protocol to move data over a network or among languages, if necessary. Flatbuffers supports partial reading/ deserialization (intended for network games; must be fast).
...
\vspace{0.25 cm}
\uncover<2->{\textcolor{darkblue}{\large Formal class structure, long names $\to$ not for end-users!}}
\end{frame}
...
\item Provides a ``reduced output'' format for a \textcolor{darkorange}{\bf Query Service} without tying it to a particular histogramming library.

\item All histograms are $n$-dimensional and sliced/projected/rebinned like Numpy.

\item Choice of \textcolor{darkorange}{\bf Flatbuffers}:

\vspace{0.25 cm}
\begin{itemize}\setlength{\itemsep}{0.25 cm}
\item {\large Minimal wire protocol, can be selectively deserialized.}
\item {\large Schema evolution: we can add classes as needed.}
\item {\large Multilingual: supports 12 languages, but primarily C++.}
\end{itemize}
\end{itemize}
\end{frame}
